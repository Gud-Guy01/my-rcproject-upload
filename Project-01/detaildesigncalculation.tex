...
According to  (EN 1990:2002, Cl. 4.1.1 \cite{EN1990-2002(5)}) Permanent loads are defined as loads whose variation in magnitude over time is negligible.
The unit weights of construction materials used to calculate these loads are given in (EN 1991-1-1:2002, Annex A, Table A.1 \cite{EN1991-1-1-2002(3)}) and (EN 1991-1-1:2025, Annex A \cite{EN1991-1-1-2025(2)}):
\begin{table}[H]
\centering
\caption{Dead Load Typical Value}
\begin{tabular}{LL}
\toprule
\textbf{Source}                        & \textbf{Typical Value} \\
\midrule
Reinforced concrete slab (200mm thick) & \( 0.2 \times  25 = 5.0 \text{ kN/m}^2   \)                      \\
Screed/floor finish                    & \( 1.5 \text{ kN/m}^2   \)                        \\
Ceiling + services (MEP)               &  \( 0.5 \text{ kN/m}^2   \)                      \\
Partitions (treated as permanent)      &   \( 1,0 \text{ kN/m}^2   \)                     \\
\textbf{Total \(G_k\)}                    &  \(8.0 \text{ kN/m}^2   \)\\\bottomrule
\end{tabular}
\end{table}
